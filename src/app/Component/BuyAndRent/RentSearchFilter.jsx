...
import { rentContainer2, } from '@/app/Data'
...
const RentSearchFilter = () => {
  const {setSearchedLettingsProperties, setSearchedRentLocation, searchedRentLocation} = useGlobalContext();
    ...
    const onSubmit = async (data, e) => {
      ...
      try {
        ...
        if (queryString) {
          ...
          navigate(`/search-properties?${queryString}`);
  ...
  return (
   ...
   <SearchInput2 clearInputField={clearInputField} title={'Rent'} />
   </section>
   ...
   <section className='bg-white w-full grid grid-cols-3 gap-[1vw] lg:grid-cols-5 lg:gap-0'>
      {rentContainer2?.map((item, index) => (
        <div className="flex items-center w-full cursor-pointer max-w-[35vw] lg:max-w-[20vw] border-[1px]" key={index}>
          <span className='text-[3.5vw] xl:text-[1.5vw] lg:text-[1.5vw] ml-[4.5vw] lg:ml-[0.5vw] md:text-[1.5vw] text-blue-950'>{item?.icon}</span>
          <Controller 
            control={control}
            name={item?.name}
            render={({ field }) => (
              <select value={''} {...field} onChange={handleSearch} className='w-full bg-inherit -ml-0.5vw cursor-pointer text-[3.5vw] xl:text-vw lg:text-vw md:text-vw font-medium text-black p-vw focus:outline-none appearance-none'>
   ...
   <aside className='w-full flex bg-white ml-vw mt-2vw md:mt-0.5vw p-vw'>
        ...
        <div className="mt-0.5vw  w-full">
          <select value={''} onChange={handleSearch} name="propertyType" className="w-full bg-inherit -ml-0.5vw cursor-pointer text-[2.5vw] xl:text-vw lg:text-vw md:text-vw font-medium text-black p-[1.5vw] md:p-vw focus:shadow-md focus:outline-none appearance-none border-[0.1vw] border-[#152347] rounded-md ">
            ...
            {propertiesType?.map((suggestion, index) => (
              <option key={index} className='text-[3.5vw] xl:text-vw lg:text-vw md:text-vw' value={suggestion}>{suggestion}</option>
            ))}
          </select>
        </div>
      </section>
    ...
    <div className={`w-full bg-gray-100 ml-2vw ${showFilter ? 'mt-vw' : 'mt-2vw'}`}>
      <button type='submit' className='text-[3vw] bg-amber-400 text-white hover:bg-amber-500 xl:text-vw lg:text-vw md:text-vw p-vw w-fit cursor-pointer rounded-[0.4vw]'>Search</button>
      <Link href='/search-properties'>
      <button className='text-[3vw] bg-[#152347] text-white hover:bg-[#1c2f5f] xl:text-vw ml-2vw lg:text-vw md:text-vw p-vw w-fit cursor-pointer rounded-[0.4vw]'>Advance Search</button>
      </Link>
    </div>
    </form>
  )
